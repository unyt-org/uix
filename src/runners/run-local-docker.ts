...
export default class LocalDockerRunner implements UIXRunner {
	...
	async run(runOptions: runOptions) {
		...
		const content = await this.generateDockerComposeFile(runOptions, deploymentDir);
	...
	async generateDockerComposeFile({baseURL, endpoint, domains, params, options, volumes}: runOptions, deploymentDir: Path) {
		...
		const volumeNames = [dxCacheVolumeName, localStorage];
		...
		// add custom persistent volumes
		let c = 0;
		for (const path of volumes??[]) {
			const volumeName = name.replace(/[^a-zA-Z0-9_.-]/g, '-') + '-persistent-' + (c++).toString()
			volumeNames.push(volumeName);
			volumeMappings.push(`${volumeName}:${new Path(path).normal_pathname}`)
		}


		// create volumes
		await Promise.all(volumeNames.map(name=>this.createVolume(name)))
		...
		const dockerCompose = {
			...
			volumes: Object.fromEntries(volumeNames.map(name=>[name, {external: true}])),
	...
	protected createVolume(name: string) {
		return execCommand(`docker volume inspect ${name} &>/dev/null || docker volume create ${name}`);
	}
