...
export namespace HTMLUtils {
    ...
    function setAttribute(element: Element, attr:string, val:unknown, root_path?:string|URL) {
        ...
        // video src => srcObject
        else if (element instanceof globalThis.HTMLVideoElement && attr === "src" && globalThis.MediaStream && val instanceof MediaStream) {
    ...
    export function setElementAttribute<T extends Element>(element:T, attr:string, value:Datex.CompatValue<any>|Function, root_path?:string|URL):boolean|Promise<boolean> {
        value = value?.[JSX_INSERT_STRING] ? value.val : value; // collapse safely injected strings

        // first await, if value is promise
        if (value instanceof Promise) return value.then(v=>setElementAttribute(element, attr, v, root_path))
        ...
        // default
        else return setAttribute(element, attr, value, root_path)
    }
