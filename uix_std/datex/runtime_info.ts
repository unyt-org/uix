...
export class DatexRuntimeInfo<O extends UIX.Components.Base.Options = UIX.Components.Base.Options> extends UIX.Components.Base<O> {
    ...
    override async onCreate(){
        ...
        const table = UIX.HTMLUtils.createHTMLElement('<table class="no-style"></table>');
        table.style.marginTop = "20px";

       
        table.append(UIX.HTMLUtils.createHTMLElement(`<tr><td style='width:9em;color:var(--text_color_highlight)'>Version</td><td>${Datex.Runtime.VERSION}</td></tr>`))
        table.append(UIX.HTMLUtils.createHTMLElement(`<tr><td style='color:var(--text_color_highlight)'>Running on</td><td>${Datex.Runtime.HOST_ENV}</td></tr>`))
        

        table.append(UIX.HTMLUtils.createHTMLElement(`<tr><td style='color:var(--text_color_highlight)'>Endpoint</td><td style='font:14px Menlo, Monaco, "Courier New", monospace'>${await MonacoHandler.colorize(Datex.Runtime.endpoint.toString(), 'datex')}</td></tr>`))
        ...
        let options_tr = UIX.HTMLUtils.createHTMLElement('<tr><td>Options</td></tr>');
        options_tr.style.color = 'var(--text_color_highlight)'
        let options_td = UIX.HTMLUtils.createHTMLElement('<td></td>');
        ...
        let compile_time_tr = UIX.HTMLUtils.createHTMLElement('<tr><td>Compile Time [ms]</td></tr>');
        compile_time_tr.style.color = 'var(--text_color_highlight)'
        let compile_time_td = UIX.HTMLUtils.createHTMLElement('<td></td>');
        compile_time_td.style.overflow = "visible";
        compile_time_td.style.paddingBlockStart = "21px";

        compile_time_tr.append(compile_time_td)
        table.append(UIX.HTMLUtils.createHTMLElement('<tr style="height:30px"></tr>'));
        table.append(compile_time_tr)

        // Allocated Pointers
        let allocated_pointers_td = UIX.HTMLUtils.createHTMLElement(`<td style='font:14px Menlo, Monaco, "Courier New", monospace'>${Datex.Pointer.pointers.size}</td>`);
        let allocated_pointers_tr = UIX.HTMLUtils.createHTMLElement(`<tr><td style='color:var(--text_color_highlight)'>Allocated Pointers</td></tr>`)
