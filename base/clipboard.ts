...
export namespace Clipboard {
	...
	export function putString(string: string){
		let data = [new ClipboardItem({ "text/plain": new Blob([string], { type: "text/plain" })})];
		return navigator.clipboard.write(data)
	}

	export function putHTML(html: string|HTMLElement){
		let data = [new ClipboardItem({"text/html": new Blob([html instanceof HTMLElement ? html.outerHTML : html], { type: "text/html" })})];
		return navigator.clipboard.write(data)
	}
	...
	async function putDatex(type:string, datex:ArrayBuffer) {
		try {
			let data = [new ClipboardItem({["web " + type]: new Blob([datex], { type: "web " + type })})];
			return await navigator.clipboard.write(data)
		} 
		// web prefix not supported
		catch (e) {
			let data = [new ClipboardItem({"text/plain": new Blob([type+"::"+arrayBufferToBase64(datex)], { type: "text/plain" })})];
			return await navigator.clipboard.write(data)
		}
	}
	...
	export async function getItem(types?:string[]):Promise<any> {
		let item = (await getItems(types))[0];
	...
	const typeHandlers:{[type:string]:(blob:Blob, text:string)=>Promise<any>|any} = {
		'text/plain': async function (blob:Blob, text:string) {
			return text;
		},
		...
		'application/datex-value': async function (blob:Blob, text:string) {
			return await Datex.Runtime.decodeValue(await blob.arrayBuffer())
		}
	...
	async function getItems<T extends string[] = ["text/plain"]>(types:T = <T>Object.keys(typeHandlers)):Promise<Record<T[number], any>[]> {
		...
		try {
			...
			// fake text/plain item, if only readText supported
				...
				items = [{getType:()=>{return <Promise<Blob>><unknown>{text:()=>navigator.clipboard?.readText()}}, types:['text/plain']}]
			}                
		} 
		catch (e) {
			return [];
