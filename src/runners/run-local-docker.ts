...
export default class LocalDockerRunner implements UIXRunner {
	...
	async run(runOptions: runOptions) {
		...
		// first check if a docker-compose file already exists, that does not start with the comment "# This file was auto generated by the uix docker plugin."
		if (filePath.fs_exists && !Deno.readTextFileSync(filePath.normal_pathname).startsWith("# This file was auto generated by the uix docker plugin.")) {
			const updatedFilePath = deploymentDir.getChildPath(`docker-compose-new.yml`);
			logger.info(`File ${filePath.normal_pathname} was manually edited, creating a new file ${updatedFilePath.normal_pathname} instead.`);
			await Deno.writeTextFile(updatedFilePath.normal_pathname, content);
		}
		else {
	...
	async generateDockerComposeFile({baseURL, endpoint, domains, params, options, volumes}: runOptions, deploymentDir: Path) {
		...
		return `# This file was auto generated by the uix docker plugin. Remove this comment to modify this file.\n\n${json2yaml(JSON.stringify(dockerCompose))}` 
	}
